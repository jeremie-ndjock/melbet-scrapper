...
def row(g=1, t=1, param="0", odds="1.500", blocked=False, is_center=False):
    return SnapshotRow(GAME, g, t, Decimal(param), Decimal(odds) if odds is not None else None,
...
def test_first_sighting_writes_every_selection():
    ...
    assert {c.key for c in changes} == {(1, 1, Decimal(0)), (1, 3, Decimal(0))}
...
def test_disappeared_selection_is_reported_as_removed():
    ...
    removed = changes[0]
    assert removed.key == (1, 3, Decimal(0)) and removed.odds is None
...
def test_removed_selection_is_not_rewritten_on_later_cycles():
    ...
    assert len(first_removal) == 1 and first_removal[0].key == (1, 3, Decimal(0))
